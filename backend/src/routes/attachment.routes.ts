...
router.get('/task/:taskId', authenticateToken, async (req: Request, res: Response) => {
  try {
...
router.get('/download/:filename', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    // Check access
    const isMember = attachment.task.project.members.some((m: any) => m.userId === userId);
    ...
    const file = await localStorageService.getFile(filename);
...
router.delete('/:id', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    await localStorageService.deleteFile(attachment.filename);
